...
# We have all the same setup. Used 3 attributes.
# n= individual
# i = item
# j = attribute
...
######################################################################################
#####################################################################################
## USEd 100 items for practice purpose
N <- nrow(Y)  
I <- nrow(Q)
K <- ncol(Q)
jags.data <- list("N", "I", "K", "Y", "Q")
#################################################################
# removed unused data
################################################################
jags.parameters <- c("lamda0","lamda1","lamda2", "lamda3", "lamda12","lamda13","lamda23","lamda123","theta","lamda","ppp")  
...
hgdina <- jags(data = jags.data, inits = jags.inits, parameters.to.save = jags.parameters,
            model.file = jags.hgdina, n.chains = 2, n.iter = 20000, n.burnin = 10000,
            n.thin = 1, DIC = TRUE)
time2 <- as.POSIXlt(Sys.time())
use.time <- difftime(time2, time1, units = "secs")

######################################################################################
#Outputs: Theta is included
######################################################################################
hgdina1 <- hgdina$BUGSoutput
hgdina1$summary

write.csv(hgdina1$summary, "Rep11_HDINA-20k.csv")
...
colnames(E.itempar) <- c("lamda0", "SE(lamda0)", "lamda1", "SE(lamda1)", "lamda2", "SE(lamda2)", 
                         ...
                         "lamda23", "SE(lamda23)","lamda123", "SE(lamda123)", "Theta", "SE(Theta)")
E.itempar
